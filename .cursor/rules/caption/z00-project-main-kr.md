...
새 컴포넌트가 필요할 때는 **즉시 만들지 말고** 다음을 따른다.

1. **컴포넌트가 들어갈 폴더(및 경로) 확인** — 기존 구조와 명명 규칙 확인 (예: `components/`, `app/<route>/_components/`, 공통 UI).
2. **제안한 폴더/경로를 사용자에게 제시**하고 확인을 받는다.
3. **사용자가 폴더 이름과 위치를 확인한 뒤에만** 컴포넌트를 생성한다.
